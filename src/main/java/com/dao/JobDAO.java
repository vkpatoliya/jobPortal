...
public class JobDAO {
	...
	public boolean addJobs(Jobs j) {
		...
		try {
			String sql ="insert into jobs(title,description,category,status,location,email) values(?,?,?,?,?,?)";
...
boolean f = false;
		...
		try {
			String sql ="update jobs set title=?,description=?,category=?,status=?,location=?,email=? where id=?";
